'''Mission SA Center — center-biased simulated annealing search.

Same as mission_sa.py but uses build_sa_center_waypoints() which adds
a center-proximity penalty to the SA cost function. Waypoints close to
the field center are visited earlier in the search order, reducing
expected discovery time when the marker is placed near the center.

Run with:
    python main.py --mode scan --planner sac

Tune the bias in path_planning/simulated_annealing_center.py:
    PENALTY_STRENGTH = 1.0   (default — raise to increase center bias)
'''
...
def build_grid_waypoints() -> list:
    '''Raw boustrophedon grid — passed to SA for reordering.'''
    waypoints   = []
...
def save_snapshot(frame, corners, ids, marker_id, log_timestamp, suffix=""):
    ...
    logger.info(f"[SAC] Snapshot → {filename}")


def run_mission(lock, marker_confirmed, ugv_signal, hover_reached, cfg,
                log_timestamp, uncertain_pos, planner, rgb_frame_mutex):
    '''Center-biased SA mission process.

    Same structure as mission_sa.py but uses build_sa_center_waypoints()
    so waypoints near the field center are visited earlier in the sweep.
    '''
    logger.info("[SAC] Process starting")
    ...
    # 1. Run center-biased SA pre-flight — north_max and east_max must match
    #    FIELD_CONFIG above so the field center is computed correctly.
    logger.info("[SAC] Running center-biased pre-flight optimisation...")
    grid      = build_grid_waypoints()
    waypoints = build_sa_center_waypoints(
        grid,
        start_pos=(0.0, 0.0),
        north_max=FIELD_CONFIG["north_max_m"],
        east_max=FIELD_CONFIG["east_max_m"],
    ...
    logger.info("[SAC] === OPTIMIZED PATH (center-biased) ===")
    for i, (n, e) in enumerate(waypoints):
        logger.info(f"[SAC]   WP {i + 1:>2}/{total} → N={n:.1f} E={e:.1f}")
    logger.info("[SAC] === END PATH ===")

    # 2. Arm and take off
    logger.info("[SAC] Arming and taking off")
    ...
    logger.info("[SAC] Airborne — starting mission loop")

    # 3. SITL detection — no OAK-D on Mac
    ...
    if not sitl_mode:
        try:
            ...
            logger.warning(f"[SAC] oak_rgb unavailable: {e} — ArUco disabled")
            sitl_mode = True

    if sitl_mode:
        logger.info("[SAC] SITL mode — position from LOCAL_POSITION_NED")
    ...
    logger.info(
        f"[SAC] Search starting — {total} waypoints, "
    ...
    try:
        while phase != "done":
            ...
            if not sitl_mode and not marker_confirmed.is_set():
                ...
                if ids is None:
                    ...
                    for mid in matched:
                        ...
                        logger.debug(
                            f"[SAC] ID={mid} consec={count}/{CONFIRM_THRESHOLD}"
                        )

                        if count >= CONFIRM_THRESHOLD:
                            # Enforce minimum 5 second flight time
                            airborne_time = time.time() - takeoff_start
                            if airborne_time < MIN_FLIGHT_TIME_S:
                                logger.info(
                                    f"[SAC] Min flight time not met "
            ...
            if phase == "sweep":
                if wp_idx < total:
                    ...
                    if elapsed >= FIELD_CONFIG["wp_dwell_s"]:
                        dist_from_target = dist2d((cur_north, cur_east), (tgt_n, tgt_e))
                        if wp_idx + 1 < total:
                            next_n, next_e = waypoints[wp_idx + 1]
                            logger.info(
                                f"[SAC] Departing WP {wp_idx + 1}/{total} | "
                            ...
                            logger.info(
                                f"[SAC] Departing WP {wp_idx + 1}/{total} — final | "
                                f"from N={cur_north:.2f} E={cur_east:.2f} | "
                    ...
                    logger.info("[SAC] Search complete — marker not found")
                    phase = "done"
            ...
            # Fly to confirmed marker position at lower altitude before landing
                ...
                if dist <= FIELD_CONFIG["wp_accept_radius_m"]:
                    logger.info(f"[SAC] Over marker at {FIELD_CONFIG['confirm_alt_m']}m")
                    phase = "land"
                elif time.time() - approach_start > FIELD_CONFIG["approach_timeout_s"]:
                    logger.warning("[SAC] Approach timeout — landing anyway")
                    phase = "land"

            # Landing — uncomment AprilTag precision landing when ready
            elif phase == "land":
                logger.info("[SAC] Stationary landing")
        ...
        logger.warning("[SAC] Interrupted")

    finally:
        log_event(csv_path, "uav_end")
        if shm_rgb is not None:
            shm_rgb.close()
        state.close()
        logger.info("[SAC] Process exiting")